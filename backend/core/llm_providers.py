...
class SensenovaProvider(LLMProvider):
    ...
    def __init__(self, api_key: str, model_name: str = "sensenova-lite", **kwargs):
        super().__init__(api_key, model_name, **kwargs)
    ...
    def call(self, prompt: str, input_data: Any = None, **kwargs) -> LLMResponse:
        ...
        try:
            ...
            logger.error(f"商汤日日新调用失败: {str(e)}")
            raise
    ...
    def get_available_models(self) -> List[ModelInfo]:
        ...
        return [
            ...
            ModelInfo(
                name="sensenova-pro",
                display_name="日日新 Pro",
                provider=ProviderType.SENSENOVA,
                max_tokens=16384,
                description="商汤日日新专业版模型"
            ),
            ModelInfo(
                name="sensenova-plus",
                display_name="日日新 Plus",
                provider=ProviderType.SENSENOVA,
                max_tokens=32768,
                description="商汤日日新增强版模型"
            )
        ]
