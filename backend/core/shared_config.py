...
class Settings(BaseModel):
    ...
    sensenova_api_key: Optional[str] = ""
...
class ConfigManager:
        ...
        if config_file.exists():
            try:
                with open(config_file, 'r', encoding='utf-8') as f:
                    ...
                    # 处理嵌套格式（api.provider 等）
                    if 'api' in config_data:
                        api_config = config_data['api']
                        if 'api_provider' in api_config:
                            self.settings.llm_provider = api_config['api_provider']
                        if 'api_model' in api_config:
                            self.settings.model_name = api_config['api_model']
                        if 'api_keys' in api_config:
                            api_keys = api_config['api_keys']
                            if 'dashscope' in api_keys:
                                self.settings.dashscope_api_key = api_keys['dashscope']
                            if 'openai' in api_keys:
                                self.settings.openai_api_key = api_keys['openai']
                            if 'gemini' in api_keys:
                                self.settings.gemini_api_key = api_keys['gemini']
                            if 'siliconflow' in api_keys:
                                self.settings.siliconflow_api_key = api_keys['siliconflow']
                            if 'sensenova' in api_keys:
                                self.settings.sensenova_api_key = api_keys['sensenova']
                    
                    # 处理扁平格式（直接 key-value）
                    for key, value in config_data.items():
                        if key not in ['api', 'basic', 'service', 'processing', 'logs', 'paths']:
                    ...
                    if 'processing' in config_data:
                        ...
                        if 'processing_chunk_size' in proc_config:
                            self.settings.chunk_size = proc_config['processing_chunk_size']
                        if 'processing_min_score' in proc_config:
                            self.settings.min_score_threshold = proc_config['processing_min_score']
                        elif 'processing_min_score_threshold' in proc_config:
                            self.settings.min_score_threshold = proc_config['processing_min_score_threshold']
                        if 'processing_max_clips' in proc_config:
                            self.settings.max_clips_per_collection = proc_config['processing_max_clips']
                        elif 'processing_max_clips_per_collection' in proc_config:
                            self.settings.max_clips_per_collection = proc_config['processing_max_clips_per_collection']
    ...
    def get_api_config(self) -> APIConfig:
        """获取API配置"""
        # 根据提供商返回对应的 API key
        provider = getattr(self.settings, 'llm_provider', 'dashscope')
        api_key = self.settings.dashscope_api_key
        
        if provider == 'openai':
            api_key = self.settings.openai_api_key or self.settings.dashscope_api_key
        elif provider == 'gemini':
            api_key = self.settings.gemini_api_key or self.settings.dashscope_api_key
        elif provider == 'siliconflow':
            api_key = self.settings.siliconflow_api_key or self.settings.dashscope_api_key
        elif provider == 'sensenova':
            api_key = self.settings.sensenova_api_key or self.settings.dashscope_api_key
        
        # 如果没有配置任何 API key，尝试从环境变量获取
        if not api_key:
