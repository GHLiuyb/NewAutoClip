...
def generate_project_thumbnail(project_id: str, video_path: Path) -> Optional[str]:
    ...
    try:
        ...
        # 简化版本：直接从视频第1秒提取一帧
        output_path = video_path.parent / f"temp_thumbnail_{video_path.stem}.jpg"
        ...
        logger.info(f"为项目 {project_id} 生成缩略图: {video_path}")
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        ...
        if result.returncode == 0 and output_path.exists():
            ...
            logger.info(f"缩略图生成成功: {project_id}")
            return f"data:image/jpeg;base64,{base64_data}"
        else:
            logger.warning(f"缩略图生成失败，跳过: {result.stderr}")
            return None
            
    except Exception as e:
        logger.warning(f"缩略图生成异常，跳过: {e}")
        return None
