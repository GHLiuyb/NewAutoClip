...
async def upload_files(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"项目 {project_id} 直接处理任务已启动（后台线程）")
            
        except Exception as e:
            logger.error(f"启动项目 {project_id} 处理任务失败: {str(e)}")
            # 即使处理任务启动失败，也要返回项目创建成功
            # 用户可以通过重试按钮重新启动处理
    ...
    try:
        ...
        def run_processing():
            try:
                ...
                logger.error(f"重试处理项目 {project_id} 时出错: {e}")
        
        # 在后台线程中运行处理
        process_thread = threading.Thread(target=run_processing, daemon=True)
        process_thread.start()
        logger.info(f"项目 {project_id} 重试处理任务已启动（后台线程）")
...
async def get_collection_thumbnail(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取合集封面失败: {str(e)}")@router.post("/upload-batch")
async def upload_batch_files(
    video_files: List[UploadFile] = File(...),
    video_category: Optional[str] = Form(None),
    project_service: ProjectService = Depends(get_project_service)
