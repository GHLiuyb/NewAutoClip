...
class ApiKeys(BaseModel):
    ...
    sensenova: str = Field(default="", description="商汤日日新API密钥")
    ...
    try:
        ...
        if settings_file.exists():
            try:
                ...
                # 检查是否是扁平格式还是嵌套格式
                if "api" not in saved_settings and "llm_provider" in saved_settings:
                    # 扁平格式，需要转换为嵌套格式
                    print("检测到扁平格式，转换为嵌套格式")
                    
                    # 构建嵌套格式
                    nested_settings = {
                        "basic": BasicSettings().dict(),
                        "service": ServiceSettings().dict(),
                        "api": {
                            "api_keys": {
                                "dashscope": saved_settings.get("dashscope_api_key", ""),
                                "openai": saved_settings.get("openai_api_key", ""),
                                "gemini": saved_settings.get("gemini_api_key", ""),
                                "siliconflow": saved_settings.get("siliconflow_api_key", ""),
                                "sensenova": saved_settings.get("sensenova_api_key", ""),
                                "jimeng_access": "",
                                "jimeng_secret": ""
                            },
                            "api_provider": saved_settings.get("llm_provider", "dashscope"),
                            "api_model": saved_settings.get("model_name", "qwen-plus"),
                            "api_max_tokens": saved_settings.get("max_tokens", 4096),
                            "api_timeout": saved_settings.get("api_timeout", 30)
                        },
                        "processing": {
                            "processing_chunk_size": saved_settings.get("chunk_size", 5000),
                            "processing_min_score": saved_settings.get("min_score_threshold", 0.7),
                            "processing_max_clips": saved_settings.get("max_clips_per_collection", 5),
                            "processing_max_retries": 3
                        },
                        "logs": LogSettings().dict(),
                        "paths": {
                    ...
                    settings = DesktopSettings(**nested_settings)
                else:
                    # 已经是嵌套格式
                    print(f"从文件读取的设置: {saved_settings.get('basic', {}).get('app_name', 'unknown')}")
                    settings = DesktopSettings(**saved_settings)
                
                # 将文件中的配置同步到 DesktopConfig 对象
                if settings.api:
                    config.current_provider = settings.api.api_provider
        ...
        settings = DesktopSettings(
            ...
            api=ApiSettings(
                api_keys=ApiKeys(
                    ...
                    sensenova=getattr(config, 'sensenova_api_key', ''),
                    jimeng_access="",  # 默认值
                    jimeng_secret=""   # 默认值
                ),
                api_provider=config.current_provider,
...
async def update_settings(settings: DesktopSettings):
    ...
    try:
        ...
        config.sensenova_api_key = settings.api.api_keys.sensenova
        config.current_provider = settings.api.api_provider
        config.default_model = settings.api.api_model
        ...
        # 同步更新LLMManager内存中的配置，使处理流程立即生效
        try:
            from backend.core.llm_manager import get_llm_manager
            from backend.core.llm_providers import ProviderType
            llm_manager = get_llm_manager()
            provider_type = ProviderType(config.current_provider)
            api_key = getattr(config, f"{config.current_provider}_api_key", "")
            if api_key:
                llm_manager.set_provider(provider_type, api_key, config.default_model)
        except Exception as e:
            logger.warning(f"同步LLMManager配置失败: {e}")
        
        # 同步更新项目data/settings.json（扁平衡格式，供ConfigManager和后续处理使用）
        try:
            import json
            from pathlib import Path
            project_data_settings = Path(__file__).parent.parent.parent.parent / "data" / "settings.json"
            flat_settings = {
            ...
            api_key_mapping = {
                "dashscope": settings.api.api_keys.dashscope,
                "openai": settings.api.api_keys.openai,
                "gemini": settings.api.api_keys.gemini,
                "siliconflow": settings.api.api_keys.siliconflow,
                "sensenova": settings.api.api_keys.sensenova,
            }
            for provider, key in api_key_mapping.items():
                flat_settings[f"{provider}_api_key"] = key
            project_data_settings.parent.mkdir(parents=True, exist_ok=True)
            with open(project_data_settings, 'w', encoding='utf-8') as f:
                json.dump(flat_settings, f, ensure_ascii=False, indent=2)
        except Exception as e:
            logger.warning(f"同步项目data/settings.json失败: {e}")
        
        # 设置环境变量，确保后续新进程可读取
        os.environ["AUTOCLIP_DEFAULT_PROVIDER"] = config.current_provider
        os.environ["API_MODEL_NAME"] = config.default_model
...
class GetModelsRequest(BaseModel):
    ...
    api_key: Optional[str] = Field(default=None, description="API密钥（可选，用于动态获取模型列表）")


@router.post("/available-models")
async def get_available_models(request: GetModelsRequest):
    """根据提供商获取可用的模型列表"""
    check_desktop_mode(relaxed=True)
    
    try:
        from backend.core.llm_providers import (
            ProviderType, LLMProviderFactory, 
            DashScopeProvider, OpenAIProvider, 
            GeminiProvider, SiliconFlowProvider,
            SensenovaProvider
        )
        ...
        try:
            ...
            if request.api_key:
                if provider_type == ProviderType.DASHSCOPE:
                    provider = DashScopeProvider(api_key=request.api_key)
                elif provider_type == ProviderType.OPENAI:
                    provider = OpenAIProvider(api_key=request.api_key)
                elif provider_type == ProviderType.GEMINI:
                    provider = GeminiProvider(api_key=request.api_key)
                elif provider_type == ProviderType.SILICONFLOW:
                    provider = SiliconFlowProvider(api_key=request.api_key)
                elif provider_type == ProviderType.SENSENOVA:
                    provider = SensenovaProvider(api_key=request.api_key)
                else:
                    provider = None
                
                if provider:
                    model_info_list = provider.get_available_models()
                    models = [
                        {
                            "name": m.name,
                ...
                # 如果没有API密钥，返回预设的模型列表
                models = _get_default_models_for_provider(request.provider)
        except Exception as e:
            logger.warning(f"动态获取模型列表失败，使用预设列表: {e}")
            models = _get_default_models_for_provider(request.provider)
        
        return {"models": models, "provider": request.provider}
        
    except ValueError as e:
        raise HTTPException(status_code=400, detail=f"无效的提供商类型: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"获取模型列表失败: {str(e)}")


def _get_default_models_for_provider(provider: str) -> List[Dict[str, Any]]:
    """获取提供商的预设模型列表"""
    default_models = {
        "dashscope": [
...
async def get_current_provider():
    ...
    try:
        ...
        # 获取实际保存的提供商（默认为 dashscope）
        current_provider = config.current_provider or "dashscope"
        
        # 根据提供商返回对应的显示信息
        provider_display_info = {
            "dashscope": {"display_name": "通义千问", "description": "阿里云通义千问服务"},
            "openai": {"display_name": "OpenAI", "description": "OpenAI 服务"},
            "gemini": {"display_name": "Google Gemini", "description": "Google Gemini 服务"},
            "siliconflow": {"display_name": "硅基流动", "description": "硅基流动服务"},
            "sensenova": {"display_name": "商汤日日新", "description": "商汤日日新服务"}
        }
        
        info = provider_display_info.get(current_provider, {"display_name": "未知提供商", "description": ""})
