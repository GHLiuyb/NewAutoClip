...
export const settingsApi = {
  ...
  // 获取可用模型列表（支持按提供商和API密钥动态获取）
  getAvailableModels: (provider: string, apiKey?: string): Promise<{ models: any[], provider: string }> => {
    return api.post('/settings/available-models', { 
      provider, 
      api_key: apiKey 
    })
  },
...
export const projectApi = {
  ...
  // 批量上传视频文件
  uploadBatchFiles: async (videoFiles: File[], videoCategory?: string): Promise<{
    total: number,
    success: number,
    failed: number,
    ...
    return api.post('/projects/upload-batch', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    })
