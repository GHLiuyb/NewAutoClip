...
export const SimpleProgressBar: React.FC<SimpleProgressBarProps> = ({
  ...
  useEffect(() => {
    if (autoStart && projectId) {
      addPollingProject(projectId, pollingInterval)
      
      return () => {
        removePollingProject(projectId)
      }
    }
  }, [projectId, autoStart, pollingInterval, addPollingProject, removePollingProject])
...
export const BatchProgressBar: React.FC<BatchProgressBarProps> = ({
  ...
  useEffect(() => {
    if (autoStart && projectIds.length > 0) {
      addPollingProjects(projectIds, pollingInterval)
      
      return () => {
        removePollingProjects(projectIds)
      }
    }
  }, [projectIds, autoStart, pollingInterval, addPollingProjects, removePollingProjects])
